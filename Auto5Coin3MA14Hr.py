...
def cal_target1(ticker):
    df = pyupbit.get_daily_ohlcv_from_base(ticker, base=14)
    yesterday = df.iloc[-2]
    today = df.iloc[-1]
...
def get_ma3(ticker):
    df = pyupbit.get_daily_ohlcv_from_base(ticker, base=14)   #3, 5, 10, 15 change
...
while True:
    ...
    try:
        ...
        # 14:00:20 ~ 14:00:30 target, ma3 Update
        if now.hour == 14 and now.minute == 0 and (20 <= now.second <= 30):
            BTC_target1 = cal_target1("KRW-BTC")
            ETH_target1 = cal_target1("KRW-ETH")
            ADA_target1 = cal_target1("KRW-ADA")
            TRX_target1 = cal_target1("KRW-TRX")
        ...
        # Next Day 13:59:30 ~ 13:59:40 ALL Sell (Market Price)
        if now.hour == 13 and now.minute == 59 and (30 <= now.second <=40):
            if op_mode is True and BTC_hold is True:
                btc_balance = upbit.get_balance("KRW-BTC")
                upbit.sell_market_order("KRW-BTC", btc_balance)  # * 0.9995 fee deltet
